 ...
 *
 * Example:
 * [vehicle ACE_player, ACE_player, [0,0,0]] call grad_trenches_legacy_fnc_bpzBuild
 *
 * Public: No
 */
...
[{
    ...
    // save performance
    if ((_vehicle getVariable [QGVAR(plowlowered), 0] == 0) && !(_vehicle getVariable [QGVAR(isDigging), false])) exitWith {};

    // init digging
    if (!(_vehicle getVariable [QGVAR(isDigging), false])) then {

        // can dig vehicle on this position
        if (!([_vehicle modelToWorld [trenchHorizontalOffset,_distanceToTrench,0]] call EFUNC(common,canDig))) exitWith {};
        ...
        if (_speed > 1) then {
            ...
            [_trench, 0] call EFUNC(common,setTrenchProgress);
