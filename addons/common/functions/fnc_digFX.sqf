 ...
 *
 * Example:
 * [TrenchObj] call grad_trenches_common_fnc_digFX;
 *
 * Public: No
 */
...
private _selections = (_trench selectionNames "Memory") select {(_x find "fxsource") isEqualTo 0};

{
    private _selectionName = _x;

    private _particleSource = "#particlesource" createVehicleLocal [0,0,0];
    private _position = _trench modelToWorld (_trench selectionPosition _selectionName);
